...
DEBUG = True # Prints parameter and tracking information
DEBUG_VERBOSE = False # Prints parameters every time they are changed by slider
...
class BallTracker:
    ...
    """
    def __init__(self, deviceOrFile=0, trackBall=True, logFile=None, camWidth=640,
        camHeight=480, pauseVideoFrames=False, trackingParameters=dict()):
        ...
        self.TrackingParameters = TrackingParameters(**trackingParameters)
        if (DEBUG):
    ...
    def run(self):
        ...
        haveNewFrame = False # when new frames are grabbed
        self.TrackingParameters.ParameterChanged = True # for first iteration
        ...
        if (self.usingImage()):
            ...
            self.Cap.release()
        if (self.usingCamera()):
            self.VideoWaits = False

        # Main Loop
        while(self.IsRunning):
            if (not self.usingImage()):
                if (not self.usingImage() and self.VideoWaits and not wantNextFrame):
                    #heldFrame = True
                    pass
                else:
                    ret, frame = self.Cap.read()
            ...
            if (self.usingVideo()):
                if (self.VideoWaits and not haveNewFrame):
                    pass
                else:
                    frame_i += 1
            ...
            if (found):
                ...
                if (DEBUG #and not (self.usingVideo() and heldFrame)
                    and (self.TrackingParameters.ParameterChanged or haveNewFrame)):
                ...
                if (DEBUG #and not (self.usingVideo() and heldFrame)
                    ...
                    print "Did not find ball{}".format(ending)
            ...
            if (key == ord('q')): # q quits
                break
            if (key == ord('n')): # n next skips to the next video frame if paused
                wantNextFrame = True
            if (key == ord('p')): # p prints all parameters
                self.TrackingParameters.printParameters()
            if (key == 13): # enter pauses a video frame
                self.VideoWaits = self.VideoWaits ^ 1
...
if __name__ == "__main__":
    ...
    # Tracking parameters
    # Painted tennis ball with webcam:
    paintedTennisBallParams = { 
        'filterIterations': 2,
        'filterSigma': 7,
        'hueLower': 34,
        'hueUpper': 54,
        ...
        'houghMinRadius': 3,
        'houghThreshLower': 21,
        'houghThreshUpper': 142,
        }

# tennis_ball2-cut.mov
    tennisBall2Params = { 
        'hueLower': 27,
        'hueUpper': 42,
        'satLower': 58,
    ...
    tracker = BallTracker(deviceOrFile=args[0], trackingParameters=tennisBall2Params)
    tracker.run()
